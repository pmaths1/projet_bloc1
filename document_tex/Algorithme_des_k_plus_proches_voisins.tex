...
\PY{k}{def} \PY{n+nf}{distance} \PY{p}{(}\PY{n}{a}\PY{p}{,} \PY{n}{b}\PY{p}{)}\PY{p}{:}
        ...
        \PY{k}{for} \PY{n}{i} \PY{o+ow}{in} \PY{n+nb}{range} \PY{p}{(}\PY{l+m+mi}{\PY{k}{len}(a)}\PY{p}{)}\PY{p}{:}
...
Nous prendrons de cette base de données un extrait de 5000 données
uniquement, afin de limiter les temps de calcul de notre algorithme.

Il nous faut obtenir deux listes, \textbf{data} et \textbf{target}.

\begin{itemize}
    \tightlist
    \item
    \textbf{data} contient les images sous forme d'une liste de 28*28 =
    784 entiers compris entre 0 et 255 correspondant aux différentes
    nuances de gris (255 étant blanc et 0 noir)
    \item
    \textbf{target} contient les chiffres (de type int) correspondant à
    l'image
\end{itemize}
\begin{enumerate}
    \item  Tout d'abord on récupère la base de donnée complète (cela peut prendre
beaucoup de temps) :
\begin{code}
    \begin{Verbatim}[commandchars=\\\{\}]
...
\item Ensuite on en prend l'extrait que l'on met sous la forme qui nous
convient (on aura besoin de 1000 autres images plus tard, d'où les 6000)
:
...
{\color{incolor}In [{\color{incolor} }]:} \PY{k+kn}{import} \PY{n+nn}{numpy} \PY{k}{as} \PY{n+nn}{np}
        ...
        \PY{n}{sample} \PY{o}{=} \PY{n}{np}\PY{o}{.}\PY{n}{random}\PY{o}{.}\PY{n}{randint}\PY{p}{(}\PY{l+m+mi}{70000}\PY{p}{,} \PY{n}{size}\PY{o}{=}\PY{l+m+mi}{6000}\PY{p}{)}
...
\item    Afficher la liste correspondant à la 23ème image du jeu de données. A
...
\item Appliquer cet algorithme à image1 et image2, et vérifier si
l'algorithme a trouvé un voisin qui représente le même chiffre.

\begin{code}
\begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor} }]:} \PY{n}{indice1} \PY{o}{=} \PY{n}{lePlusProcheVoisin}\PY{p}{(}\PY{n}{image1}\PY{p}{)}
\PY{n+nb}{print}\PY{p}{(}\PY{n}{target}\PY{p}{[}\PY{n}{indice1}\PY{p}{]}\PY{p}{)}
\PY{n+nb}{print}\PY{p}{(}\PY{n}{target}\PY{p}{[}\PY{n}{i1}\PY{p}{]}\PY{p}{)}

\PY{n}{indice2} \PY{o}{=} \PY{n}{lePlusProcheVoisin}\PY{p}{(}\PY{n}{image2}\PY{p}{)}
\PY{n+nb}{print}\PY{p}{(}\PY{n}{target}\PY{p}{[}\PY{n}{indice2}\PY{p}{]}\PY{p}{)}
\PY{n+nb}{print}\PY{p}{(}\PY{n}{target}\PY{p}{[}\PY{n}{i2}\PY{p}{]}\PY{p}{)}
\end{Verbatim}
\end{code}

On remarque éventuellement que ce n'est pas forcément parfait, le
résultat n'étant pas toujours celui attendu.
...
{\color{incolor}In [{\color{incolor}  }]:} \PY{k}{def} \PY{n+nf}{lesKplusProchesVoisins} \PY{p}{(}\PY{n}{image}\PY{p}{,} \PY{n}{k}\PY{p}{)}\PY{p}{:}
             ...
             \PY{k}{for} \PY{n}{i} \PY{o+ow}{in} \PY{n+nb}{range} \PY{p}{(}\PY{n}{k}\PY{p}{)}\PY{p}{:}
                 ...
                 \PY{k}{for} \PY{n}{j} \PY{o+ow}{in} \PY{n+nb}{range} \PY{p}{(}\PY{n+nb}{len} \PY{p}{(}\PY{n}{data}\PY{p}{)}\PY{p}{)}\PY{p}{:}
                     \PY{k}{if} \PY{n}{listeDesDistances} \PY{p}{[}\PY{n}{j}\PY{p}{]} \PY{o}{!=} \PY{l+m+mi}{0} \PY{o+ow}{and} \PY{n}{listeDesDistances} \PY{p}{[}\PY{n}{j}\PY{p}{]} \PY{o}{\PYZlt{}} \PY{n}{p} \PY{o+ow}{and}
                      \PY{n}{j} \PY{o+ow}{not} \PY{o+ow}{in} \PY{n}{Kppv}\PY{p}{:}
...
\item
Exécuter l'algorithme avec image1 et k=5, puis donner la liste des
chiffres associés aux images dont l'algorithme donne les indices dans
notre base.
\end{enumerate}
...
Prenons maintenant un jeu d'images à tester, qui sont en dehors de la
base des 5000 images traitées par l'algorithme.



Notre but est de calculer le taux d'erreur des prédictions, puis de
chercher la meilleure valeur de \emph{k} à choisir pour diminuer
celui-ci.

\begin{enumerate}
    \item
    Écrire une fonction qui calcule le taux d'erreur sur les prédictions
    portant sur le jeu d'images à tester. Exécuter ensuite cette fonction
    pour une valeur de \emph{k} choisie entre 2 et 7.
...
\end{code} 
\item
Écrire une fonction optimisation(n) qui détermine la valeur de
\emph{k}, entre 1 et n, qui minimise le taux d'erreur, puis donner le
résultat de cette fonction avec n=7.
\begin{code}
\begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor} }]:} \PY{k}{def} \PY{n+nf}{optimisation}\PY{p}{(}\PY{n}{n}\PY{p}{)}\PY{p}{:}
\PY{l+s+sd}{\PYZsq{}\PYZsq{}\PYZsq{}}
...
\PY{n+nb}{print}\PY{p}{(}\PY{n}{optimisation}\PY{p}{(}\PY{l+m+mi}{7}\PY{p}{)}\PY{p}{)}
\end{Verbatim}
\end{code} 
\end{enumerate}
    \end{document}
